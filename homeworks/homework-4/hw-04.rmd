...
lambdac = rgamma(sim, shape = prior$a, rate = prior$b)

mus = sapply(sqrt(1/(prior$c*lambdas)),rnorm, n = 1, mean = prior$m)
muc = sapply(sqrt(1/(prior$c*lambdac)),rnorm, n = 1, mean = prior$m)
...
ggplot(data = simDF, aes(x = mu, y = lambda, 
  ...
  theme(plot.title = element_text(hjust = 0.5))
```
Because there seems to be a statistically significant amount of outliers, this means our prior assumption is not reasonable.
Additionally, The graphical representation of our prior assumption confirms with our prior beliefs, specifically that there seems to be no significant relationship between  mean change in IQ score and treatment groups-which consist of the control group and spurters(special group). As for both groups there seems to be a wide range of changes in the mean IQ score,but treatment does NOT seem to affect the mean change in IQ score at all. This is because our prior beliefs are that there is no relationship between treatment and mean change in IQ score.
