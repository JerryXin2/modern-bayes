...
$$\theta\exp(-\theta x)\frac{b^a}{\Gamma(a)}\theta^{a-1}\exp(-b\theta)$$
...
$$\theta^{a}\exp(-\theta *(b + x))$$
$$\Ga(\theta|a+1,b + x)$$
The posterior is a Gamma Distribution with Gamma(a + 1, b + x).
...
plot(gamma.sim, informativeDen1, lty = 1, axes = FALSE, xlab = '', ylab = '',
 type = 'l', , col = c("green"))
par(new = TRUE)
